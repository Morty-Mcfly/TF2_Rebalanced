...
::GTFW_PreWepFix <- function(exists, ID)
{
	...
	local baseitem	= TF_WEAPONS_BASE(tf_class, slot, className, itemID, ammo_type, clip, reserve, w_model, v_model, wearable, clear_stats, func, class_arms)
...
::CheckAttr <- function()
{
	...
	{
		local wep = NetProps.GetPropInt(GetListenServerHost().GetActiveWeapon(), "m_AttributeManager.m_Item.m_AttributeList.m_Attributes.00"+i+".m_iAttributeDefinitionIndex")
		local wep_val = NetProps.GetPropInt(GetListenServerHost().GetActiveWeapon(), "m_AttributeManager.m_Item.m_AttributeList.m_Attributes.00"+i+".m_iRawValue32")
		printl(format("%i %i %i",i,wep,wep_val) )
	}
	for (local i = 10; i < 20; i++)
	{
		local wep = NetProps.GetPropInt(GetListenServerHost().GetActiveWeapon(), "m_AttributeManager.m_Item.m_AttributeList.m_Attributes.0"+i+".m_iAttributeDefinitionIndex")
		local wep_val = NetProps.GetPropInt(GetListenServerHost().GetActiveWeapon(), "m_AttributeManager.m_Item.m_AttributeList.m_Attributes.0"+i+".m_iRawValue32")
		printl(format("%i %i %i",i,wep,wep_val) )
	}
}
